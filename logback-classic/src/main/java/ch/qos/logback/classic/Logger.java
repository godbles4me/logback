...
public final class Logger
    ...
    // 当前目录(category)的双亲. 虽有的目录都至少有一个祖先---根目录.
    transient private Logger parent;

    // 当前日志器的孩子节点. 一个日志器可能有0或多个子节点.
     ...
     *
     * Logger组件委托AppenderAttachableImpl类实现调用Appender组件记录日志.
     */
     ...
     */
    // 是否向上级logger传递日志信息(是否重复打印)
    transient private boolean additive = true;

    // Logger上下文,与Logger互相引用
